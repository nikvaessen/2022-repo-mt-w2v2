...
def _compute_error_rates(
    ...
    sorted_indexes_with_thresholds = sorted(
        [(index, threshold) for index, threshold in enumerate(predicted_scores)],
        key=itemgetter(1),
    )
    sorted_indexes = [t[0] for t in sorted_indexes_with_thresholds]
    thresholds = [t[1] for t in sorted_indexes_with_thresholds]

    groundtruth_scores = [groundtruth_scores[int(i)] for i in sorted_indexes]
    fnrs: List[float] = []
    fprs: List[float] = []
