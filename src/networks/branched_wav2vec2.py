...
class BranchedWav2Vec2Network(pl.LightningModule):
    def __init__(
        self,
    ...
    def forward(
        ...
        wav2vec2_embeddings1, wav2vec2_embeddings2 = self.compute_wav2vec2_embeddings(
            audio_features, attention_mask
        )

        return wav2vec2_embeddings1, wav2vec2_embeddings2, num_audio_features
    ...
    def compute_wav2vec2_embeddings(
        ...
        encoder_output1, encoder_output2 = self.wav2vec2.encoder(
            input_token_sequence, attention_mask=attention_mask
        )

        embedding1 = encoder_output1.last_hidden_state
        embedding2 = encoder_output2.last_hidden_state

        # embedding should be of shape [BATCH_SIZE, NUM_FRAMES, NUM_FEATURES]
        bs, num_frames, num_features = embedding1.shape
        ...
        bs, num_frames, num_features = embedding2.shape
        assert bs == input_token_sequence.shape[0] == attention_mask.shape[0]
        assert num_frames == input_token_sequence.shape[1] == attention_mask.shape[1]
        assert num_features == self.num_embedding_features
