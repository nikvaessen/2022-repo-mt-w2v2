...
class Wav2vec2(t.nn.Module):
    def __init__(
        ...
        random_permutation_mode: Optional[str] = None,
        ...
        if branch_idx is not None and random_permutation_mode is not None:
            raise ValueError(
        ...
        if self.config.do_stable_layer_norm:
            if random_permutation_mode is not None:
                self.encoder = Wav2vec2RandomPermutationEncoderStableLayerNorm(
                    cfg,
                    enable_gradient_checkpointing=enable_gradient_checkpointing,
                    pretrained_weights=pretrained_weights,
                    shuffle_input_sequence=random_permutation_mode,
                )
